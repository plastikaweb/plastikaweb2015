...
$mail->AddAddress('login@email.com');
...
if (!empty($fields)) {
    ...
    if (empty($returnVal->messages)) {                         // Enable encryption, 'ssl' also accepted
        ...
        $datepicker = stripslashes(safe($fields['datepicker']));
        $hour = stripslashes(safe($fields['hour']));
        $size = stripslashes(safe($fields['size']));
        // The sender of the form/mail
        $mail->From = $email;
        $mail->FromName = 'Guest';
        $mail->Subject = '[' . $company_name . ']';
        $content = $email . " sent you a message. He would like to book for the date:<br>";
        $content .= "<br><strong>" . $datepicker . "</strong> at <strong>" . $hour . "</strong><br>
         a table for <strong>". $size ."</strong><br><br><br>Email: " . $email;
        $mail->Body = $content;
